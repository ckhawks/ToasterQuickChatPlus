...
public static class MainMenuQuickChatSettings
{
    ...
    public static void Create()
    {
        ...
        root.style.height = new StyleLength(new Length(100, Length.Unit.Percent));
        root.style.width = new StyleLength(new Length(100, Length.Unit.Percent));
        ...
        closeButton.RegisterCallback<MouseEnterEvent>(new Action<MouseEnterEvent>((evt) =>
        {
            closeButton.style.backgroundColor = Color.white;
            closeButton.style.color = Color.black;
        }));
        closeButton.RegisterCallback<MouseLeaveEvent>(new Action<MouseLeaveEvent>((evt) =>
        {
            closeButton.style.backgroundColor = new StyleColor(new Color(0.25f, 0.25f, 0.25f));
            closeButton.style.color = Color.white;
        }));
        closeButton.RegisterCallback<ClickEvent>(new Action<ClickEvent>(QuickChatPlusSettingsCloseButtonClickHandler));
        ...
        Il2CppSystem.Collections.Generic.List<string> quickChatsSortedAsStrings = TUtils.ToIl2CppList(QuickChats.GetQuickChatsAlphabeticalAsStrings());
        Il2CppSystem.Collections.Generic.List<string> visibilityOptions = new Il2CppSystem.Collections.Generic.List<string>();
        ...
        for (int i = 0; i < 40; i++)
        {
            quickChatsScrollView.Add(CreateQuickChatRow(
                ...
                quickChatsSortedAsStrings.IndexOf(QuickChats.quickchats[Plugin.configQuickChatIndex[i].Value]),
                visibilityOptions, 
                Plugin.configQuickChatVisibility[i].Value
            ));
        }
    ...
    // 
    public static VisualElement CreateQuickChatRow(int slotIndex, Il2CppSystem.Collections.Generic.List<string> quickChatsSortedAsStrings, int selectedIndexFromSorted, Il2CppSystem.Collections.Generic.List<string> visibilityOptions, int selectedVisibility)
    {
        ...
        quickChatDropdownField.RegisterCallback<ChangeEvent<string>>(
            new Action<ChangeEvent<string>>(evt =>
            {
                // Plugin.Log.LogInfo($"Changed on slot index {slotIndex} from {evt.previousValue} to {evt.newValue}");
                Plugin.configQuickChatIndex[slotIndex].Value = QuickChats.GetQuickChatByName(evt.newValue).id;
        ...
        visibilityDropdownField.RegisterCallback<ChangeEvent<string>>(
            new Action<ChangeEvent<string>>(evt =>
            {
                ...
                {
                    Plugin.configQuickChatVisibility[slotIndex].Value = 1;
                }
                if (evt.newValue == "ALL")
                {
                    Plugin.configQuickChatVisibility[slotIndex].Value = 0;
                }
            })
        );
